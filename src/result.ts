...
export class Result<T, E> {
  ...
  static combine(resultList: readonly Result<unknown, unknown>[]): Result<unknown, unknown> {
  ...
  map<A>(f: (t: T) => A): Result<A, E> {
    return this.isOk() ? Result.ok(f(this.data.value!)) : Result.err(this.data.error!)
  }

  mapErr<U>(f: (e: E) => U): Result<T, U> {
    return this.isErr() ? Result.err(f(this.data.error!)) : Result.ok(this.data.value!)
  }
  ...
  andPush<R extends Result<unknown, unknown>, U extends T extends readonly unknown[] ? T : [T]>(
    ...
    return result.map((v) => [...(Array.isArray(value) ? (value as U) : ([value] as U)), v])
  }
